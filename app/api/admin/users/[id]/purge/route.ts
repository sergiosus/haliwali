...
import { purgeSoftDeletedUser } from "../../../../../lib/serverUserSoftDelete";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await purgeSoftDeletedUser({
  ...
  if (result === "not_due") return NextResponse.json({ error: "NOT_DELETED" }, { status: 409 });
