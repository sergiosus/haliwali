...
import type { Listing, ServiceListing } from "../../lib/listings";
import { generateEditToken, useListingsStore } from "../../lib/listings";
import { moderateListing } from "../../lib/moderation";
import { categoryToSlug, serviceCategories } from "../../lib/categories";
...
export default function PostServicePage() {
  ...
  async function addService(
    form: {
      title: string;
      description: string;
      categoryName: (typeof serviceCategories)[number];
      city: string;
      address?: string;
      latitude?: number;
      longitude?: number;
    ...
    const listing: Listing = {
      ...
      description: form.description,
      categoryName: form.categoryName,
      categorySlug: categoryToSlug(form.categoryName, "service"),
    ...
    await addListing(listing as ServiceListing);
    submittedRef.current = true;
        ...
        (async () => {
          const run = async () => {
            ...
            try {
              ...
              const catSlug = categoryToSlug(categoryName, "service");
              const attrs = sanitizeListingAttributesForListing(
                { categoryName, categorySlug: catSlug, type: "service" },
                attributes,
              );
              await onSubmit({
                title: title.trim(),
    ...
    >
      ...
      <div className="max-w-xl">
        <Field label="Категория">
          <select
            value={categoryName}
            onChange={(e) => setCategoryName(e.target.value as (typeof serviceCategories)[number])}
            className={[
      ...
      <ListingAttributesFields
        key={categoryToSlug(categoryName, "service")}
